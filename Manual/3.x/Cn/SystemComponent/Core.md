...
创建swooleServer服务,注册服务回调事件以及执行***EasySwooleEvent***中的***mainServerCreate***事件：

```php
function createServer()
```

注册crontab进程和fastCache进程，启动服务：
...
注册事件回调函数；注册默认的pipe通讯；注册默认的worker start：
